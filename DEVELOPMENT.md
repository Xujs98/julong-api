...
| --- | --- | --- | --- |
| 2026-07-26 | 修复开发热更新后邮件设置持续显示旧 404：前端配置查询升级缓存键并在每次挂载时强制重新获取；后端为旧前端仍在调用的 `GET/PUT /api/email-settings` 增加兼容别名，避免浏览器保留旧模块时缺少 `/config` 导致 404。 | `EmailAlertSettings`、`GET/PUT /api/email-settings`、`router/email_settings_router_test.go`、`DEVELOPMENT.md` | 路由兼容别名与标准 `/config` 注册回归测试；前端类型、构建和格式检查见本次提交。 |
